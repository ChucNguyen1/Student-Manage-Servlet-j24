                    ...
                    <form action="${baseURL}/admin/hocky-list" method="GET" class="d-flex justify-content-end">
                       <div class="input-group" style="width: 350px;">
                         <input type="text" name="searchKey" class="form-control" 
                                placeholder="Tìm theo tên học kỳ hoặc năm học..." value="${param.searchKey}">
                    ...
                    <tbody>
                        ...
                        <c:forEach var="hk" items="${dsHocKy}">
                            <tr>
                              ...
                              <td><span class="badge bg-info text-dark">x${hk.heSo}</span></td>
                              <td>
                              ...
                              <td>
                                <div class="form-check form-switch">
                                  <input class="form-check-input" type="checkbox" 
                                         id="switch-${hk.maHK}" 
                                         ${hk.trangThai ? 'checked' : ''}
                                         onchange="toggleStatus('${hk.maHK}', this)">
                                  <label class="form-check-label" for="switch-${hk.maHK}">
              ...
              <div class="row align-items-center mt-3">
                  <div class="col-md-6">
                      <span class="text-muted">
                          ...
                          <c:if test="${totalItems > 0}">Hiển thị <b>${startItem}</b> đến <b>${endItem}</b> trong <b>${totalItems}</b> học kỳ</c:if>
                      </span>
                  </div>
                  <div class="col-md-6">
                      <nav aria-label="Page navigation">
                          <ul class="pagination justify-content-end mb-0">
                              <li class="page-item ${currentPage <= 1 ? 'disabled' : ''}">
                                <a class="page-link" href="${baseURL}/admin/hocky-list?page=${currentPage - 1}&searchKey=${param.searchKey}">Trước</a>
                              </li>
                              <c:forEach var="i" begin="1" end="${totalPages > 0 ? totalPages : 1}">
                                  <li class="page-item ${i == currentPage ? 'active' : ''}">
                                    <a class="page-link" href="${baseURL}/admin/hocky-list?page=${i}&searchKey=${param.searchKey}">${i}</a>
                                  </li>
                              </c:forEach>
                              <li class="page-item ${currentPage >= totalPages ? 'disabled' : ''}">
                                <a class="page-link" href="${baseURL}/admin/hocky-list?page=${currentPage + 1}&searchKey=${param.searchKey}">Sau</a>
                              </li>
                          </ul>
                      </nav>
                  </div>
          ...
          <form action="${baseURL}/admin/hocky-add" method="POST">
            ...
            <div class="modal-body">
              <div class="mb-3">
                  ...
                  <input type="text" class="form-control" name="tenHK" required placeholder="Ví dụ: Học kỳ 1">
              </div>
              <div class="mb-3">
                  <label class="form-label">Hệ Số <span class="text-danger">*</span></label>
                  <input type="number" class="form-control" name="heSo" value="1" min="1" required>
                  <div class="form-text">Thường là 1 cho HK1, 2 cho HK2</div>
              </div>
              <div class="mb-3">
                  <label class="form-label">Thuộc Năm Học <span class="text-danger">*</span></label>
        ...
        <div class="modal-content">
          <form action="${baseURL}/admin/hocky-edit" method="POST">
            ...
            <div class="modal-body">
              ...
              <div class="mb-3">
                  <label class="form-label">Tên Học Kỳ <span class="text-danger">*</span></label>
                  <input type="text" class="form-control" id="tenHK_edit" name="tenHK_edit" required>
              </div>
              <div class="mb-3">
                  <label class="form-label">Hệ Số <span class="text-danger">*</span></label>
                  <input type="number" class="form-control" id="heSo_edit" name="heSo_edit" min="1" required>
              </div>
              <div class="mb-3">
                  <label class="form-label">Thuộc Năm Học <span class="text-danger">*</span></label>
...
<script>
  ...
  function toggleStatus(maHK, checkbox) {
      ...
      fetch('${baseURL}/admin/hocky-status?id=' + maHK + '&status=' + newStatus, { method: 'GET' })
      .then(response => {
          if (!response.ok) {
