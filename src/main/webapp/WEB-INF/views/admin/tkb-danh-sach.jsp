...
<main id="main" class="main">
    ...
    <div class="pagetitle">
      <h1><i class="bi bi-calendar3"></i> Quản lý Thời Khóa Biểu</h1>
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="${baseURL}/">Trang chủ</a></li>
          <li class="breadcrumb-item active">Danh sách lớp cần xếp TKB</li>
        </ol>
      </nav>
    </div>
    ...
    <section class="section">
      <div class="row">
        ...
        <div class="col-12">
          <div class="card">
            <div class="card-body">
              ...
              <form action="${baseURL}/admin/tkb" method="GET" id="filterForm">
                <div class="row g-3">
                ...
                <div class="text-center mt-3">
                  ...
                  <a href="${baseURL}/admin/tkb" class="btn btn-secondary">
        ...
        <!-- Bảng danh sách lớp TKB -->
        <div class="col-12">
          <div class="card">
            <div class="card-body">
              <h5 class="card-title">
                <i class="bi bi-table"></i> Danh sách lớp thời khóa biểu
              </h5>
                    ...
                    <table class="table table-bordered table-hover align-middle">
                      <thead class="table-primary">
                        <tr class="text-center">
                          ...
                          <th style="width: 18%">Tiến độ xếp TKB</th>
                        ...
                        <c:forEach var="item" items="${danhSach}" varStatus="status">
                          ...
                          <tr>
                            ...
                            <td class="text-center">
                              <span class="badge bg-success">${item.soTietDaXep}</span>
                              <span class="text-muted mx-1">/</span>
                              <span class="badge bg-secondary">${item.tongSoTiet}</span>
                              <small class="text-muted d-block mt-1">
                                (${item.soTietChuaXep} tiết chưa xếp)
                              </small>
                            </td>
                            <td class="text-center">
                              <c:choose>
                                <c:when test="${item.daXepDayDu}">
                                  <span class="badge bg-success">
                                    <i class="bi bi-check-circle-fill"></i> Hoàn tất
                                  </span>
                                </c:when>
                                <c:when test="${item.soTietDaXep > 0}">
                                  <span class="badge bg-warning">
                                    <i class="bi bi-hourglass-split"></i> Đang xếp
                                  </span>
                                </c:when>
                                <c:otherwise>
                                  <span class="badge bg-danger">
                                    <i class="bi bi-x-circle-fill"></i> Chưa xếp
                                  </span>
                                </c:otherwise>
                              </c:choose>
                            </td>
                            <td class="text-center">
                              <a href="${baseURL}/admin/tkb-detail?maNH=${item.maNH}&maKhoi=${item.maKhoi}&maLop=${item.maLop}&maHK=${item.maHocKy}" 
                                 class="btn btn-primary btn-sm">
                                <i class="bi bi-calendar2-week"></i> Xếp TKB
                              </a>
                            </td>
                          </tr>
                        </c:forEach>
                        ...
                        <nav aria-label="Phân trang">
                          <ul class="pagination justify-content-end mb-0">
                            <li class="page-item ${currentPage <= 1 ? 'disabled' : ''}">
                              <a class="page-link" href="${baseURL}/admin/tkb?page=${currentPage - 1}&maNH=${param.maNH}&maHK=${param.maHK}&maKhoi=${param.maKhoi}&maLop=${param.maLop}">Trước</a>
                            </li>
                            <c:forEach var="i" begin="1" end="${totalPages}">
                              <li class="page-item ${i == currentPage ? 'active' : ''}">
                                <a class="page-link" href="${baseURL}/admin/tkb?page=${i}&maNH=${param.maNH}&maHK=${param.maHK}&maKhoi=${param.maKhoi}&maLop=${param.maLop}">${i}</a>
                              </li>
                            </c:forEach>
                            <li class="page-item ${currentPage >= totalPages ? 'disabled' : ''}">
                              <a class="page-link" href="${baseURL}/admin/tkb?page=${currentPage + 1}&maNH=${param.maNH}&maHK=${param.maHK}&maKhoi=${param.maKhoi}&maLop=${param.maLop}">Sau</a>
                            </li>
                          </ul>
                        </nav>
                      </c:if>
